...
function sendStudentName() {
    ...
    if (id == null || id === "")
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data === "wrongData")
                    studentDetailsDiv.insertAdjacentHTML('beforeend', `<div class='red-warning-small mt-4' role='alert'>AisId does not match with your name or surname</div>`);
                else if (data === "studentExamInserted") {
                    location.href = "/Final/views/fillTest.php";
                } else if (data === "alreadyFinished") {
                    studentDetailsDiv.insertAdjacentHTML('beforeend', `<div class='red-warning-small mt-4' role='alert'>You have already finished this test</div>`);
                }
...
function registerNewTeacher() {
    ...
    if (nickname === null || nickname === "")
        document.getElementById("teacherRegistration").insertAdjacentHTML('beforeend', `<div class='alert alert-danger' role='alert'>Name is required</div>`);
    else if (psw === null || psw === "")
        document.getElementById("teacherRegistration").insertAdjacentHTML('beforeend', `<div class='alert alert-danger' role='alert'>Password is required</div>`);
        ...
        if (psw === psw2) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data === "alreadyRegistered") {
                        document.getElementById("teacherRegistration").insertAdjacentHTML('beforeend', `<div class='red-warning-small mt-4' role='alert'>Account already exists</div>`);
                    } else
            ...
            document.getElementById("teacherRegistration").insertAdjacentHTML('beforeend', `<div class='red-warning-small mt-4' role='alert'>Passwords do not match</div>`);
        }
    }
}
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data === "wrongPassword") {
                document.getElementById("teacherLogin").insertAdjacentHTML('beforeend', `<div class='red-warning-small mt-4' role='alert'>Incorrect password</div>`);
            } else
